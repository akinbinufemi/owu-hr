...
export class FileManager {
  ...
  /**
   * Load backup data from file (supports both JSON and ZIP formats)
   */
  async loadBackupFile(filePath: string): Promise<BackupData> {
    try {
      ...
      if (fileExtension === '.zip') {
        ...
        return await this.extractBackupFromZip(filePath);
   ...
   */
  private async extractBackupFromZip(zipFilePath: string): Promise<BackupData> {
    ...
    try {
      ...
      // Find the main backup JSON file
      let backupEntry = null;
      for (const entry of zipEntries) {
        if (entry.entryName.startsWith('backup_') && entry.entryName.endsWith('.json')) {
          backupEntry = entry;
          break;
        }
      }
      ...
      const jsonContent = backupEntry.getData().toString('utf8');
      const backupData = JSON.parse(jsonContent);
      
      return backupData;
    } catch (error) {
      console.error('Failed to extract ZIP file:', error);
      throw new Error(`Failed to extract backup from ZIP: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  validateUploadedFile(file: Express.Multer.File): { valid: boolean; error?: string } {
    try {
      ...
      if (!hasValidMimeType && !hasValidExtension) {
        return {
          valid: false,
          error: `Invalid file type. Only JSON and ZIP files are allowed. Received: ${file.mimetype} (${file.originalname})`
        };
      }
